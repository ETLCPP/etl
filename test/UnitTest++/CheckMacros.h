...
#define CHECK_MESSAGE_IF(b, m1)  \
  {                              \
    if (b)                       \
      std::cerr << (m1) << "\n"; \
  }
#define CHECK_MESSAGE1_IF(m1)    \
  {                              \
    if (b)                       \
      std::cerr << (m1) << "\n"; \
  }
#define CHECK_MESSAGE2_IF(m1, m2)        \
  {                                      \
    if (b)                               \
      std::cerr << (m1) << (m2) << "\n"; \
  }
#define CHECK_MESSAGE3_IF(m1, m2, m3)            \
  {                                              \
    if (b)                                       \
      std::cerr << (m1) << (m2) << (m3) << "\n"; \
  }
#define CHECK_MESSAGE4_IF(m1, m2, m3, m4)                \
  {                                                      \
    if (b)                                               \
